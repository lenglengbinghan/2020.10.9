...
public class OtherSphere : MonoBehaviour
{
    ...
    {
        Debug.Log("爱我别走");
        if (collision.gameObject.name == "upwall" || collision.gameObject.name == "downwall")
        {
            currentV = Vector3.Reflect(currentV, Vector3.forward);
        }
        if (collision.gameObject.name == "leftwall" || collision.gameObject.name == "rightwall")
        {
            currentV = Vector3.Reflect(currentV, Vector3.right);
        }
